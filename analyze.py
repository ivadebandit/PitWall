...
def get_h2h_career(driver1, driver2, locations, years):
    ...
    for location in locations:
        ...
        for year in years:
            try:
                session = get_session(year, location, 'Q', light=True)
                d1_row = session.results[session.results['Abbreviation'] == driver1]
                d2_row = session.results[session.results['Abbreviation'] == driver2]
                if d1_row.empty or d2_row.empty:
                    continue
...
def get_drs_effect(session, driver, lap_number=None):
    ...
    telemetry = lap.get_telemetry()
    telemetry = telemetry[['DRS', 'Speed', 'Distance']].dropna()
...
def get_lap_delta(session, driver1, driver2, lap1_number=None, lap2_number=None):
    ...
    tel1= lap1.get_telemetry()[['Distance', 'Time']].dropna()
    tel2 = lap2.get_telemetry()[['Distance', 'Time']].dropna()
